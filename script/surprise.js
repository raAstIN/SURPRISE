...
(function() {
    ...
    // Add click listeners to number keys
    keyButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            if (currentPasscode.length < 6) {
                currentPasscode += btn.dataset.num;
                updateDots();
                checkPasscode();
            }
        });
    ...
    if (lampBtn) {
        lampBtn.addEventListener('click', () => {
            ...
            updateDots();
            clearError();
            const titleElement = document.querySelector('.lockscreen-content h2');
            if (titleElement) {
                titleElement.textContent = 'ناشناس';
            }
        });
    }
